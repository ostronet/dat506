...
class Game:
    ...
    def getPlayers(self):
        return self.players #returnar lista med två spelare
...
class Player:
   ...
   #HINT: It should probably take the Game that creates it as parameter and some additional properties that differ between players (like firing-direction, position and color)
    ...
    def fire(self, angle, velocity):
        ...
        rev_angle = 180 - angle if self.isReversed else angle

        x0 = self.xPos
        y0 = self.game.getCannonSize()/2
    ...
    def projectileDistance(self, proj):
        dx = proj.getX() - self.getX()
        r=self.game.getBallSize()
        längd=self.game.getCannonSize() / 2
        träff = (längd + r)
    ...
    def getX(self):
        return self.xPos #TODO: this is just a dummy value
